...
def batch(env_output, use_keys, all_tl):
    ...
    obs_batch = {}
    for i in use_keys+['mask', 'phase_index', 'masked_phase']:
        obs_batch[i] = []
    for agent_id in all_tl:
        # tmp_dict = {}
        out = env_output[agent_id]
        # for k in use_keys + ['mask']:
        #     # if k in ['green_utilization', 'green_balance', 'history_duration', 'phase_index']:
        #     #     tmp_dict[k] = out[k]
        #     # elif k == 'history_phase':
        #     #     tmp_dict[k] = out[k]
        #     # else:
        #     tmp_dict[k] = out[k]
        ...
        for k in use_keys + ['mask', 'phase_index', 'masked_phase']:
            obs_batch[k].append(out[k])
        # obs_batch['mask'].append(mask)

    for key, val in obs_batch.items():
        if key not in ['phase_index', 'mask', 'history_phase']:
...
def seq_sample(indices, batch_size):
