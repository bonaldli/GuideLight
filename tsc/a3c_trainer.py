...
def write_tensorboard(writer, loss_actor, loss_critic, loss_entropy,supervise_loss, advantages, reward, step):
    ...
    writer.add_scalar('supervise_loss', supervise_loss, step)
    if reward:
        r = {}
...
def train_worker(id, shared_NN, env_config, port, optimizer, rollout_counter, constants, device):
    ...
    # Assumes PPO worker
    local_NN = NN_Model(12, 4, state_keys=env_config['state_key'], device=device).to(device)
    ...
    while rollout_counter.get() < constants['episode']['num_train_rollouts'] + 1:
        policy_loss, entropy_loss, value_loss, supervise_loss, sampled_advantages, all_reward = worker.train_rollout(train_step)
        write_tensorboard(writer, policy_loss, value_loss, entropy_loss, supervise_loss, sampled_advantages, all_reward, rollout_counter.get())
...
def train_A3C_PPO(config, device):
    ...
    shared_NN = NN_Model(12, 4, state_keys=env_config['state_key'], device=device).to(device)
...
# def test_A3C_PPO(config, device, data_collector, loaded_model):
#     env_config = config['environment']
...
#     for i in range(config['parallel']['num_workers']):
#         id = 'test_'+str(i)
#         p = mp.Process(target=test_worker, args=(id, ep_counter, env_config, device, None, data_collector, shared_NN))
#         p.start()
#         processes.append(p)
#     for p in processes:
#         p.join()
...
if __name__ == '__main__':
    ...
    device = 'cpu'
    env_config = config['environment']
    shared_NN = NN_Model(9, 4, state_keys=env_config['state_key'], device=device).to(device)
    shared_NN.share_memory()
    optimizer = torch.optim.Adam(shared_NN.parameters(), config['ppo']['learning_rate'])
    ...
    train_worker(id, shared_NN, env_config, port,  optimizer, rollout_counter, config, device, ps_encoding)
    # for i in range(config['parallel']['num_workers']):
    #     id = i
    #     port = env_config['port_start'] + i
    #     p = mp.Process(target=train_worker, args=(id, shared_NN, env_config, port, optimizer,
